...
`@starting-style` 可作为局部分组嵌套使用；展开时必须保留父选择器和声明顺序，不能把声明直接
提升到样式表顶层。`globalStyle` 同时允许顶层和嵌套形式。`@scope` 保留原生分组体及其相对于
父样式规则的位置，以保留起点、终点、内部 `&` / `:scope` 和隐式 scope 的不同语义；产物中的
scope 由浏览器原生处理，不承诺为缺少该功能的浏览器降级。

嵌套选择器中的 `&` 表示整个父选择器集合，其特异性遵循 CSS Nesting 的 `:is()` 规则。
多个 `&` 分别引用该集合，不能按单个父分支替换而遗漏交叉组合。单个复杂父选择器也需保留
整体匹配语义；声明与嵌套规则的先后顺序不变。普通 CSS Modules 的 `@scope` 起点和终点内的
局部类名与规则选择器使用相同的映射，`:global()` / `:local()` 仍按现有 Modules 约定处理。
...
当前只折叠由单个非 atomic 类名字符串、空字符串、`false` 和 `null` 字面量组成的调用，
直接生成字符串常量。含变量引用、条件、数组、对象或嵌套表达式的调用保留原 AST 和运行时
绑定，不把参数源码拼接后作为独立表达式重解析。这样保留词法遮蔽、导入身份、TDZ、参数
求值顺序及副作用，也避免生成的 `Boolean` 引用被调用方作用域捕获。
