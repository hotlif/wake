...
文档定义预期，测试证明预期，实现满足预期。任何会改变公开或内部可观察行为、契约、诊断、持久化产物、构建或 CI 策略、架构不变量的工作，按以下顺序进行：

1. 先定位最接近的权威文档或契约。现有材料已经准确表达预期时只需引用并复核；否则先起草或更新对应材料。用户行为写入 `docs/` 或相应公开参考，局部内部不变量写入 crate rustdoc 或工程文档，长期架构意向先写 `proposed` ADR，普通未来事项写入 [ROADMAP.md](ROADMAP.md)。`proposed` ADR 不得作为当前边界或机器策略依据；描述当前状态的文档可以在工作树中先行起草，但只能随对应测试和实现组成完整变更后提交或交付，不得提前发布为已支持能力。
2. 再新增或调整表达预期的最小测试或 fixture，并运行确认它因目标缺口失败。已有精确失败测试时先复现，不重复新增同类测试。
3. 最后编写最小实现，先让聚焦测试转绿，再按本文件选择与风险匹配的门禁。完整变更交付时，文档、测试和实现必须一致。

行为保持重构先运行现有聚焦测试建立基线，仅在存在覆盖缺口时补测试。纯文档、纯测试、只读审查、格式或版本同步不要求制造失败测试。行为缺陷无法稳定复现时先构造最小复现；仍不能复现则在修改实现前报告阻塞。

## 本地临时产物

手工验证的临时脚本、配置和日志放在仓库根 `.tmp/<任务名>/` 下，不散落在 `.tmp` 根目录。
这些文件不提交；任务完成后删除已无用途的一次性脚本和源码备份，只保留仍需复查的证据。
需要长期复用的检查进入正式 `scripts/` 或测试目录，不依赖本地临时路径。

自动化脚本优先使用运行器提供的临时目录，并为每次运行分配独立子目录，在 `finally` 中清理
自己创建的资源。现有性能脚本使用 `RUNNER_TEMP`，缺省回退到根 `.tmp`，并清理本次临时 worktree。
清理前确认路径归属；不按整个 `.tmp` 递归删除其他任务的缓存或产物。`.gitignore` 只忽略根
`/.tmp/`，不隐藏源码或 fixture 内同名路径；已跟踪文件仍接受架构来源检查。
...
发布证据和 stable readiness：Windows x64 对 reviewed major 151、Linux x64 对 reviewed major 151 或 152 运行 blocking conformance；
macOS x64 在托管 runner 滚动切换期间对具备不可变官方清单证据的 reviewed major 150 或 151、
macOS arm64 对 reviewed major 150 或 152 运行 blocking 功能 smoke，
但证据明确不是 stable
...
| `browser-conformance` | 五个发布目标 / Rust 1.95 + Node 24 | Windows x64 major 151、Linux x64 reviewed major 151/152 conformance、macOS x64 reviewed major 150/151 与 arm64 reviewed major 150/152 功能 smoke、Linux ARM64 reviewed unavailable 证据 |
...
- Windows x64 执行 reviewed major 151、Linux x64 执行 reviewed major 151 或 152 的显式 browser path、CDP lifecycle、React、
  screenshot 和 coverage conformance；macOS x64 在具备不可变 runner 清单证据的 reviewed major
  150 或 151、macOS arm64 在 reviewed major 150 或 152 执行同类功能 smoke，
...
5. 发布前在五个目标平台和 Node 22.14/24/26 的 15 个组合中，以本次构建的主包、CSS 包和 matching
   ...
   Windows x64 的 reviewed major 151、Linux x64 的 reviewed major 151 或 152 执行 blocking conformance；在 macOS x64 上以 reviewed major 150 或 151、macOS arm64
   上以 reviewed major 150 或 152 执行
