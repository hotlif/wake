...
托管 runner 清单：Windows x64 对 Chromium 主版本 151、Linux x64 对已审查主版本 151 或 152 运行阻塞式一致性；
macOS x64 在托管 runner 滚动切换期间对具备不可变官方清单证据的已审查主版本 150 或 151、
macOS arm64 对已审查的主版本 150 或 152 运行阻塞式功能浏览器冒烟测试，
并明确不视为稳定一致性；
...
- 测试超时、无限循环、未处理 rejection、浏览器/宿主崩溃、取消、畸形 IPC、资源源认证和幂等关闭，
  确保不泄漏子进程、端口、页面或配置目录。
- Windows x64 验证系统浏览器精确主版本 151、Linux x64 验证已审查主版本 151 或 152，以及 CDP、React、截图与覆盖率一致性。
  macOS x64 验证有不可变 runner 清单证据的已审查精确主版本 150 或 151，macOS arm64 验证已审查的
  精确主版本 150 或 152 和相同功能冒烟，
  但标记为不满足稳定就绪一致性。
...
- 独立评估稳定浏览器就绪情况：只有 Windows x64、Linux x64/arm64 和 macOS x64/arm64 全部针对同一
  精确主版本执行一致性测试，才可得到 `ready: true`。当前架构 v3 证据必须得到 `ready: false`，
  因此 ADR 接受仍被阻塞。
...
五目标共享精确主版本浏览器就绪结果为 `ready: true`。仅通过架构 v3 的实验性发布拆分，不足以接受
本 ADR 或标记为稳定。
