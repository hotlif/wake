...
mod dev;
pub use dev::{DEV_SHARED_REQUESTS, DevModule, dev_module_key, render_for_dev_server};
...
pub fn generate_with_mode_in(
    ...
    let rendered = render_prepared(inputs, options, mode, docs_mode, None)?;
...
pub fn render_with_mode(
    ...
    let inputs = prepare_render_inputs(project_root.as_ref(), options)?;
    render_prepared(inputs, options, mode, docs_mode, None)
}
...
fn render_prepared(
    ...
    mode: BuildMode,
    docs_mode: DocsMode,
    demand: Option<Option<&BTreeSet<String>>>,
) -> Result<RenderedProject, DocsError> {
    ...
    let api_entries: Vec<_> = pages
        ...
        .collect();
    let dev_modules = if let Some(selected) = demand {
        dev::render_dev_entries(
            &mut generation,
            &pages,
            &demos,
            &options.base_path,
    ...
    let registry = if demand.is_some() {
        render_registry_with_dev(
            &pages,
            &demos,
            &api_entries,
            Some((&options.base_path, &dev_modules)),
        )
    } else {
        render_registry(&pages, &demos, &api_entries)
    };
    let site_entry = if demand.is_some() {
        format!("import '@@wake/docs/dev/host.ts';\n{RUNTIME_SITE_ENTRY}")
    } else {
        RUNTIME_SITE_ENTRY.to_owned()
    };
    ...
    let entry_relative = match docs_mode {
        DocsMode::Site => {
            fixed.push(("runtime/site-entry.tsx", site_entry.as_str()));
...
fn render_registry(
    ...
    render_registry_with_dev(pages, demos, api_entries, None)
}

fn render_registry_with_dev(
    pages: &[(PathBuf, CompiledPage)],
    demos: &[DemoInfo],
    api_entries: &[&ApiEntry],
    demand: Option<(&str, &[DevModule])>,
) -> String {
    let mut output = String::from("// Generated by Wake. Do not edit.\nexport const pages = [\n");
    let load_dev = |kind: &str, id: &str| {
        let key = dev_module_key(kind, id);
        demand
            .filter(|(_, modules)| modules.iter().any(|module| module.key == key))
            .map(|(base, _)| format!("loadDevModule({}, {})", js_string(base), js_string(&key)))
    };
    for (_, page) in pages {
        ...
        let load = load_dev("page", &page.route.id).unwrap_or_else(|| {
            format!(
                "import({})",
        ...
        output.push_str(&format!(
            "  {{ ...{meta}, routePath: {route_path}, load: () => {load} }},\n"
        ));
    }
    ...
    for demo in demos {
        ...
        if let Some(load) = load_dev("demo", &demo.id) {
            output.push_str(&format!("  {{ ...{metadata}, load: () => {load}.then(value => value.demo), loadSource: () => {load}.then(value => value.source) }},\n"));
        } else {
            output.push_str(&format!(
    ...
    output.push_str(" as const;\n");
    output.push_str(if demand.is_some() {
        "export const developmentDemand = true;\n"
    } else {
        "export const developmentDemand = false;\n"
    });
    if demand.is_some() {
        output.push_str("import { loadDevModule } from '@@wake/docs/runtime/dev-loader.mjs';\n");
    }
    output
}
...
mod tests {
    ...
    pub(super) fn write_fixture_navigation(root: &Path) {
    ...
    pub(super) fn rendered_file_snapshot(rendered: &RenderedProject) -> BTreeMap<String, Vec<u8>> {
        rendered
            .files
            .iter()
